...
<script type="text/javascript">
	function check(){
		var stu_psd=document.getElementById("adm_psd").value;
		if(stu_psd.length == 0){
			alert("请输入原密码！");
			return false;
		}
	...
	<body>
		...
		<div class="main_body">
			<div class="modpsd">
				<form action="${pageContext.request.contextPath }/mod_admin_psdServlet" method="post" onsubmit="return check()">
				<table class="modpsd_table" >
					<tr>
						<input type="hidden"  value=${sessionScope.userid}  name="adm_id" id="adm_id"/>
						<td class="psditem">原密码:</td>
						<td><input type="password" name="adm_psd" id="adm_psd" />*</td>
						<td><div class="psdmsg"></div></td>
					</tr>
					<tr>
						<td class="psditem">新密码:</td>
						<td><input type="password" name="new_psd" id="new_psd"/>*</td>
						<td><div class="psdmsg">密码长度必须是6位到12位</div></td>
					</tr>
					<tr>
